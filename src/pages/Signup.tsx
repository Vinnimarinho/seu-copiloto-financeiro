...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!agreed) { toast.error(t("auth.mustAccept")); return; }
    if (password.length < 8) { toast.error(t("auth.passwordTooShort")); return; }
    if (!isValidCpf(cpf)) { toast.error(t("auth.invalidCpf")); return; }
    setLoading(true);
    ...
    if (preError || !pre?.cpf_hash) {
      ...
      if (code === "cpf_already_in_use") {
        toast.error(t("auth.cpfInUse"));
      } else if (code === "invalid_cpf") {
        toast.error(t("auth.invalidCpf"));
      } else {
        toast.error(t("auth.cpfValidationError"));
      }
      return;
    }
    ...
    <div className="min-h-screen bg-sidebar flex items-center justify-center p-6">
      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="font-heading text-2xl font-bold text-sidebar-foreground">{t("auth.signupTitle")}</h1>
          <p className="text-sm text-sidebar-foreground/60 mt-1">{t("auth.signupSubtitle")}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 shadow-elevated space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder={t("auth.fullNamePlaceholder")} required className="w-full h-10 pl-10 pr-4 rounded-lg border border-input bg-background text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t("auth.emailPlaceholder")} required className="w-full h-10 pl-10 pr-4 rounded-lg border border-input bg-background text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
          </div>
          <div>
            <label className="text-sm font-medium text-foreground mb-1.5 block">{t("auth.cpf")}</label>
            <div className="relative">
            ...
            <p className="text-[11px] text-muted-foreground mt-1">{t("auth.cpfHelper")}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-foreground mb-1.5 block">{t("auth.password")}</label>
            <div className="relative">
              <Lock className="w-4 h-4 text-muted-foreground absolute left-3 top-1/2 -translate-y-1/2" />
              <input type={showPass ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t("auth.passwordMin")} required className="w-full h-10 pl-10 pr-10 rounded-lg border border-input bg-background text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
          ...
          <div className="flex items-start gap-2">
            ...
            <label htmlFor="terms" className="text-xs text-muted-foreground leading-relaxed">
              {t("auth.agreeTerms")} <Link to="/terms" className="text-primary hover:underline">{t("auth.termsOfUse")}</Link> {t("auth.and")} <Link to="/privacy" className="text-primary hover:underline">{t("auth.privacyPolicy")}</Link>
            </label>
          </div>
        ...
        <p className="text-center text-sm text-sidebar-foreground/60 mt-6">
          {t("auth.haveAccount")} <Link to="/login" className="text-sidebar-primary font-medium hover:underline">{t("auth.loginLink")}</Link>
        </p>
