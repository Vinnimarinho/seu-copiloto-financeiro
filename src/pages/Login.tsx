...
export default function LoginPage() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) { toast.error(t("auth.informEmail")); return; }
    setLoading(true);
  ...
  return (
    <div className="min-h-screen bg-sidebar flex items-center justify-center p-6">
      <SEO title={`${t("auth.loginTitle")} — Lucius`} description={t("auth.loginSubtitle")} path="/login" noindex />
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-sidebar-foreground/60 mt-1">
            {resetMode ? t("auth.resetSubtitle") : t("auth.loginSubtitle")}
          </p>
        </div>
        ...
        <form onSubmit={resetMode ? handleReset : handleLogin} className="bg-card border border-border rounded-xl p-6 shadow-elevated space-y-4">
          ...
          <Button variant="hero" className="w-full" type="submit" disabled={loading}>
            {loading ? t("common.wait") : resetMode ? t("auth.sendLink") : t("auth.loginBtn")}
          </Button>
          {resetMode && (
        ...
        <p className="text-center text-sm text-sidebar-foreground/60 mt-6">
          {t("auth.noAccount")} <Link to="/signup" className="text-sidebar-primary font-medium hover:underline">{t("auth.createAccount")}</Link>
        </p>
